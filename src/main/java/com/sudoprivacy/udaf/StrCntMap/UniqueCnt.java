...
@Description(
        name = "unique_cnt",
        value = "Return the number of unique str in sets as str format.",
        extended = "Example:\n > SELECT unique_cnt(col) from table;"
)
public class UniqueCnt extends AbstractGenericUDAFResolver {
    @Override
    ...
    public GenericUDAFEvaluator getEvaluator(TypeInfo[] info) throws SemanticException {
        if (1 != info.length) {
            throw new UDFArgumentLengthException("Function unique_cnt takes 1 argument");
        }
        return new MaxStrEvaluator();
    }
